...
import org.spring.springboot.tracce.brave.DubboKeys;
import org.spring.springboot.tracce.brave.util.AntPathMatcher;
import org.spring.springboot.tracce.brave.util.PatternMatcher;
import org.spring.springboot.tracce.brave.util.WebUtils;
...
public class HttpBraveServletFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (hasAlreadyFilteredAttribute) {
            ...
            if(Objects.equals(Boolean.TRUE,serverSpan.getSample())){
                Span span = serverSpan.getSpan();
                MDC.put(DubboKeys.ZIPKIN_TRACEID_MDC, convertToString(span.getTrace_id()));
                MDC.put(DubboKeys.ZIPKIN_SPANID_MDC, convertToString(span.getId()));
                if (span.getParent_id() != null) {
                    MDC.put(DubboKeys.ZIPKIN_PARENTID_MDC, convertToString(span.getParent_id()));
                } else {
                    MDC.remove(DubboKeys.ZIPKIN_PARENTID_MDC);
                }
            }
            ...
            try {
                ...
                MDC.remove(DubboKeys.ZIPKIN_TRACEID_MDC);
                MDC.remove(DubboKeys.ZIPKIN_SPANID_MDC);
                MDC.remove(DubboKeys.ZIPKIN_PARENTID_MDC);
            }
        }
    }
